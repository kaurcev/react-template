...
export default function PrivacyView(){
    const { title, appSite, appAuthorMail, appDomain } = useAppContext();
    document.title = `Политика конфиденциальности | ${title}`;
        useEffect(()=>{
            window.scrollTo(0,0);
    }, [])    
    ...
    return (
        ...
        <Header />
            <main>
                ...
                <p>Данная политика конфиденциальности относится к сайту под доменным именем {appDomain}. Эта страница содержит сведения о том, какую информацию мы ({appSite} или администрация сайта) или третьи лица могут получать, когда вы пользуетесь нашим сайтом.</p>
                ...
                <p>Если у вас есть какие-либо вопросы о политике конфиденциальности, использованию сайта или иным вопросам, связанным с сайтом, свяжитесь с нами: <a href={mailtogen(appAuthorMail)}>{appAuthorMail}</a></p>
                </div>
            </main>
        <Footer />
        </>
